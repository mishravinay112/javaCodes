...
public class balls extends Applet implements Runnable {
	...
	public void run() {
		while (true) {
			try {
				repaint();
				Thread.sleep(100); // Thread time interval set to 100 mil
									// seconds
				if (x < wi - 100) // Playing with position of balls on the
									// applet screen
					x += 5;
				if (y < he - 100)
					y += 5;
	...
	public void paint(Graphics g) {
		...
		he = d.height;
		wi = d.width;
		g.setColor(new Color(r.nextInt(255), r.nextInt(255), r.nextInt(255))); // Balls
																				// random
																				// colors
																				// sets
